...
async def generate_image(file: UploadFile = File(...), prompt: str = Form(...)):
    """
    Generates an AI-generated image based on the provided input image and text prompt.

    Args:
        file (UploadFile): The uploaded image file in RGB format.
        prompt (str): The text prompt describing the desired transformation.

    Returns:
        StreamingResponse: A PNG image generated by the model.
    
    Raises:
        HTTPException: If image generation fails due to an internal error.
    """
    try:
